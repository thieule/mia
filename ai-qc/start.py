#!/usr/bin/env python3
"""Start the Mia QC deployment (`ai-qc/`): loads .env, installs mia-ai + ai-tools, runs gateway.

Layout (siblings under the repo root):

  core/        # mia-ai (package mia)
  ai-tools/     # local MCP (registry, pytest_runner, …)
  ai-qc/        # API / UI / quality testing assistant
...
Canonical policies for this Mia QC deployment. The agent must not modify these files without explicit approval.
"""

_AGENT_README = """# Agent area (autonomous)

Test plans, API/UI run notes, repro steps, and draft reports; subject to `../admin/` policy.
"""


def _require_python() -> None:
    if sys.version_info < (3, 11):
        print("Mia QC requires Python 3.11+. Current:", sys.version, file=sys.stderr)
        sys.exit(1)
...
def _ensure_test_runs_path() -> None:
    ...
    if not os.environ.get("TEST_RUNS_PATH_MIA_QC", "").strip():
        os.environ["TEST_RUNS_PATH_MIA_QC"] = default
...
def run_pip_install(quiet: bool) -> None:
    ...
    if AI_TOOLS_ROOT.is_dir():
        print("Installing ai-tools (Mia QC default; registry + pytest MCP)...", flush=True)
...
def validate_config() -> None:
    ...
    print("Config OK (Mia QC)", flush=True)
    ...
    if not dc:
        ...
        if not dc.get("enabled"):
            print("  discord token (Mia QC): skipped (disabled)", flush=True)
        else:
            print("  discord token (Mia QC):", f"set ({len(tok)} chars)" if tok else "MISSING", flush=True)
...
def main() -> None:
    _require_python()
    parser = argparse.ArgumentParser(description="Start Mia QC gateway (mia + optional Discord).")
    parser.add_argument(
        "--validate-only",
        action="store_true",
    ...
    print("Starting Mia QC gateway (Ctrl+C to stop)...", flush=True)
    run_gateway()
