# Code, errors, and repository documentation in English (Mia QC)

**Status:** Human-maintained policy under `admin/`. Mia QC **must read and follow** this file when writing or editing **code** or **repository documentation** covered below. It **must not** edit files in `admin/` without an approved workflow.
...
- Any **markdown (or equivalent) documentation** that Mia QC **creates or updates** because **policy or task requirements** call for docs (e.g. **`DOCUMENTATION_AND_COMPLETION_CHECKLIST.md`**, **`docs/AI_PROJECT_WORKSPACE_SPEC.md`**, README updates, `projects/<slug>/docs/`, completion checklists) must be written in **English** as well.
...
| **Log messages** intended for operators (logger, `console.error`, …) added or changed by Mia QC | **English** |
...
Applies whenever Mia QC **writes or materially edits** these as part of a task or **mandatory policy** (not casual notes in chat):
