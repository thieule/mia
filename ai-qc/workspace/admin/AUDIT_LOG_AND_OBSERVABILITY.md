# Audit trail and observability (Mia QC)

**Status:** Human-maintained policy under `admin/`. Describes **what Mia QC / nanobot already records** and **how admins can review** it. Mia QC **must not** edit files in `admin/` without an approved workflow.
...
- **Retention:** `sessions/` is often **gitignored** (see `ai-qc/.gitignore`); copy snapshots to secure storage if you need **long-term compliance** archives.
