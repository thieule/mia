...
| `second_brain.git.commit` / `agile_studio.git.commit` | Stub `:Commit`–`[:HAS_COMMIT]`←`:Project` |
| *(các event khác)* | POST vẫn tới nhưng **skipped** nếu chưa map |
...
- Tool chính:
  ...
  - `brain_search_knowledge` — kNN (`search_mode=vector`) hoặc **hybrid** kNN+BM25 (`search_mode=hybrid`); lọc `scope`, `visibility`, `source_type` (`agile` / `code`).
