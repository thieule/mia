# Code, errors, and repository documentation in English (Mia tech)

**Status:** Human-maintained policy under `policy/`. Mia tech **must read and follow** this file when writing or editing **code** or **repository documentation** covered below. It **must not** edit files in `policy/` without an approved workflow.
...
- Any **markdown (or equivalent) documentation** that Mia tech **creates or updates** because **policy or task requirements** call for docs (e.g. **`DOCUMENTATION_AND_COMPLETION_CHECKLIST.md`**, **`docs/AI_PROJECT_WORKSPACE_SPEC.md`**, README updates, `projects/<slug>/docs/`, completion checklists) must be written in **English** as well.
...
| **Log messages** intended for operators (logger, `console.error`, …) added or changed by Mia tech | **English** |
...
Applies whenever Mia tech **writes or materially edits** these as part of a task or **mandatory policy** (not casual notes in chat):
...
See **`policy/README.md`**, **`policy/DOCUMENTATION_AND_COMPLETION_CHECKLIST.md`**, **`policy/TESTING_AND_DEFINITION_OF_DONE.md`**.
