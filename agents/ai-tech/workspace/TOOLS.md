...
- Prefer **writing** long-lived content to **`agent/`** (and `policy/` only when those human-maintained files say so).

## Writes and mutating commands

- **`policy/PRE_IMPLEMENTATION_APPROVAL.md`** applies: **confirm the idea and scope with an admin** before **`write_file`**, **`edit_file`**, mutating **`exec`**, or implementation **`spawn`**. Read-only tools are not gated by that flow unless policy is extended.

## Tests and “done”

- **`policy/TESTING_AND_DEFINITION_OF_DONE.md`**: implementation changes need **unit tests**; run **`pytest`** (via **`mcp_pytest_runner_*`** or **`exec`**) and report results **before** stating the task is **done**. Do not claim completion on failing or skipped runs without an admin-documented exception.
- **`policy/DOCUMENTATION_AND_COMPLETION_CHECKLIST.md`**: update **docs**, add **supplementary** material if needed, and write **`projects/<slug>/docs/COMPLETION_CHECKLIST.md`** (or repo-equivalent) **before** “done”. Template: **`docs/templates/COMPLETION_CHECKLIST.template.md`**.

## Code language (repository)

- **`policy/CODE_COMMENTS_AND_ERRORS_ENGLISH.md`**: comments, docstrings, exceptions, operator-facing errors/logs, and **markdown docs** you add under policy (README, `docs/`, checklists) must be **English**.
