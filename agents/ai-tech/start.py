#!/usr/bin/env python3
"""Start the Mia tech deployment (`agents/ai-tech/`): loads .env, installs mia-ai + ai-tools, runs gateway.
...
Layout (monorepo root = parent of ``agents/``; ``core`` và ``ai-tools`` cùng nằm trong ``agents/``):
  ...
  agents/
    ...
    ai-tech/    # triển khai này — gateway + workspace + launcher
...
Canonical policies for this Mia tech deployment. The agent must not modify these files without explicit approval.
"""

_AGENT_README = """# Agent area (autonomous)

Design drafts, investigation notes, and artefacts the agent may create; subject to the human-maintained files under `../policy/`.
"""


def _require_python() -> None:
    if sys.version_info < (3, 11):
        print("Mia tech requires Python 3.11+. Current:", sys.version, file=sys.stderr)
        sys.exit(1)
...
def _apply_second_brain_mcp_env() -> None:
    ...
    os.environ.setdefault("MIA_AGENT_ID", "mia-tech")
...
def load_dotenv(path: Path) -> None:
    ...
    # Optional; config.json can reference ${GEMINI_API_KEY} / ${GEMINI_MODEL} like OpenRouter.
...
def _ensure_test_runs_path() -> None:
    ...
    # .env may set TEST_RUNS_PATH_MIA_TECH= (empty); setdefault would not override that.
    if not os.environ.get("TEST_RUNS_PATH_MIA_TECH", "").strip():
        os.environ["TEST_RUNS_PATH_MIA_TECH"] = default
...
def run_pip_install(quiet: bool) -> None:
    ...
    if AI_TOOLS_ROOT.is_dir():
        print("Installing ai-tools (Mia tech default; registry + pytest MCP)...", flush=True)
...
def validate_config() -> None:
    ...
    print("Config OK (Mia tech)", flush=True)
    ...
    if not dc:
        ...
        if not dc.get("enabled"):
            print("  discord token (Mia tech): skipped (disabled)", flush=True)
        else:
            print("  discord token (Mia tech):", f"set ({len(tok)} chars)" if tok else "MISSING", flush=True)
...
def main() -> None:
    _require_python()
    parser = argparse.ArgumentParser(description="Start Mia tech gateway (mia + optional Discord).")
    parser.add_argument(
        "--validate-only",
        action="store_true",
    ...
    print("Starting Mia tech gateway (Ctrl+C to stop)...", flush=True)
    run_gateway()
