...
**Tên gọi chính thức:** **Mia BA** (M + BA in hoa). Thư mục triển khai: `agents/ai-ba/`.
...
Uses the same **mia** package as other deployments (`../../core` from here), **separate gateway** (default port **18793**), **separate workspace** (`workspace/`), and optional **Discord** bot token `DISCORD_BOT_TOKEN_MIA_BA`.
...
- **Governance (policy):** [workspace/policy/](./workspace/policy/) · [Audit / operations reference](./workspace/docs/OPERATIONS_AUDIT_TRAIL.md) *(not policy)*
...
- **Mia tech** (`../ai-tech/`) — engineering / code-focused line; default gateway **18792** (`agents/ai-tech/`).
- **Mia DevOps** (`../ai-devops/`) — deploy, infra, CI/CD; default gateway **18794** (`agents/ai-devops/`).
- **Mia QC** (`../ai-qc/`) — test API, UI/E2E, QC; default gateway **18795** (`agents/ai-qc/`).
- **Mia PM** (`../ai-pm/`) — dự án, ưu tiên, lịch, rủi ro, báo cáo; default gateway **18797** (`agents/ai-pm/`).
