...
> **Location:** `workspace/docs/` — part of the **Mia BA** workspace (`./workspace` in config). The agent reads this file as `docs/AI_PROJECT_WORKSPACE_SPEC.md` (including when `restrictToWorkspace` is enabled).
...
| Topic | Description |
|--------|-------------|
| **Goal** | A **predictable** directory tree when BA specs live alongside **project-specific** Git roots, **traceable** to the remotes your team actually uses. |
...
1. **Agile Studio MCP** — use **`agile_project_get`** / **`agile_projects_list`** and read fields such as **`settings.github_repository`**, **`documents_storage_path`**, **`storage_overview`**, **`workspace_ref`**. Use them as the **source of truth** for where requirements and specs should be written or cloned next to.
...
**Principle:** one **clone** of the agreed canonical remote (when BA work is tied to a repo) plus **child trees** per project/module when multiple units share one `WORK_ROOT`.
...
- **`upstream/<upstream-clone>/`**: content from **`git clone`** / **`git pull`** for the URL from §2 — do **not** change `origin` unless an admin says so.
- **`projects/<slug>/`:** BA and engineering artefacts per unit; include **`docs/`** and **`COMPLETION_CHECKLIST.md`** per team policy.

### 3.2. Option B — Submodule or subtree

If each unit is its **own** Git repo, use **submodule** or **subtree** under `projects/<slug>/`. Run such commands **only** when the user or policy explicitly allows.
...
6. If **`restrictToWorkspace: true`**, `WORK_ROOT` must sit inside the allowed workspace or policy must allow the path (see **`../docs/BA_SETUP.md`** under the `ai-ba/` deployment).
...
Optional **local-only** env (illustrative names; never commit secrets):
