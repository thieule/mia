...
- **Wiki & Requirements:**
    ...
    - Backlog Stories (Wiki: Draft): `agile-studio-2`, `agile-studio-5`, `agile-studio-6`.
    ...
    - Logic Models: Story #24 (Workflows) uses "IF [Trigger] AND [Condition] THEN [Action]"; Task #29 (PR Automation) links to #24, #28, #32. Task #28 (Commit Linking) implementation is pending approval from "bố Tony".
    ...
    - Second Brain: Conceptualized as a tool for collaboration and knowledge management. Uses Neo4j Aura (managed), Graph-RAG with Cypher, and automated ADR extraction (MADR v2.1.0). Story 6 logic uses few-shot prompting for chat/comment decision extraction. Tools: `query_graph`, `get_neighborhood`, `upsert_relation`, `search_knowledge`.
