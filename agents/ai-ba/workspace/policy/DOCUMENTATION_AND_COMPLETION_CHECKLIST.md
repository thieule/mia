# Documentation updates and completion checklist (Mia BA)
...
- [ ] **Code & tests** — if the task included **code** changes: pytest run result summary (or pointer to `run_id` / log); tests added/updated listed. If **requirements/docs only**, write **“N/A — no code in scope”** and point to doc paths delivered instead.
...
Teams may extend this list in their own template; Mia BA **must not** claim **done** with empty or missing sections when work was non-trivial.
