# Pre-implementation admin approval (Mia BA)
...
Reduce risk from autonomous action: **no durable edits** (specs, code, config, or other files in the repo / `WORK_ROOT`), **no installs**, and **no mutating shell commands** until an **admin** has **explicitly approved** the plan in writing. BA-heavy work is often **markdown and diagrams in Git** — those writes are still in scope for this gate when they use mutating tools.
