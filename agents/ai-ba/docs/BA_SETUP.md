...
- Python **3.11+**
- Sibling directories **`../../core`** (mia-ai / nanobot source) and **`../../ai-tools`** (local MCP servers) from this `docs/` folder.
- **`agents/ai-ba/.env`** (copy from `EXAMPLE_.env`)
...
| | **Mia tech** | **Mia BA (`agents/ai-ba/`)** |
|--|--------|--------|
| Gateway (default) | **18792** | **18793** |
| Config | `agents/ai-tech/config/config.json` | `agents/ai-ba/config/config.json` |
...
- **Admin gate before mutating:** workspace policy in **`../workspace/policy/PRE_IMPLEMENTATION_APPROVAL.md`** — Mia BA must get **explicit admin confirmation** before mutating tools run when policy applies.
- **Tests before “done”** (when the task changes **code**): **`../workspace/policy/TESTING_AND_DEFINITION_OF_DONE.md`**. Pure BA deliverables (requirements docs, diagrams in chat) follow **`../workspace/policy/DOCUMENTATION_AND_COMPLETION_CHECKLIST.md`**.
- **English in repo:** **`../workspace/policy/CODE_COMMENTS_AND_ERRORS_ENGLISH.md`**
- **Audit / review (operator doc, not policy):** **`../workspace/docs/OPERATIONS_AUDIT_TRAIL.md`**
