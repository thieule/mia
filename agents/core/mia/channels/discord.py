...
from mia.command.builtin import build_help_text
from mia.config.paths import get_media_dir
from mia.config.schema import Base
from mia.utils.helpers import safe_filename, split_message
...
MAX_INLINE_TEXT_ATTACHMENT_BYTES = 200 * 1024  # 200KB
MAX_INLINE_TEXT_ATTACHMENT_CHARS = 12_000
MAX_MESSAGE_LEN = 2000  # Discord message character limit
TYPING_INTERVAL_S = 8
TEXT_ATTACHMENT_SUFFIXES = {
    ".txt", ".md", ".markdown", ".json", ".jsonl", ".py", ".js", ".ts", ".tsx",
    ".jsx", ".yaml", ".yml", ".xml", ".csv", ".log", ".ini", ".cfg", ".conf",
    ".toml", ".sql", ".sh", ".ps1", ".bat",
}
...
if DISCORD_AVAILABLE:
    ...
    class DiscordBotClient(discord.Client):
        ...
        async def send_outbound(self, msg: OutboundMessage) -> None:
            """Send a mia outbound message using Discord transport rules."""
...
class DiscordChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if not DISCORD_AVAILABLE:
            logger.error("discord.py not installed. Run: pip install mia-ai[discord]")
            return
    ...
    def _should_accept_inbound(
        ...
        if not self.is_allowed(sender_id):
            logger.warning(
                "Discord: dropped message from user id {} (not in allowFrom). "
                "Set channels.discord.allowFrom to include this id.",
                sender_id,
            )
            return False
        if message.guild is not None and not self._should_respond_in_group(message, content):
            logger.info(
                "Discord: ignored (mention mode): channel={} sender={} - @mention the bot in this channel, or use a DM.",
                message.channel.id,
                sender_id,
            )
            return False
    ...
    async def _download_attachments(
        ...
        """Download supported attachments and return paths + prompt content blocks."""
        ...
        for attachment in attachments:
            ...
            if attachment.size and attachment.size > MAX_ATTACHMENT_BYTES:
                blocks.append(f"[attachment: {filename} - too large]")
                continue
            try:
                ...
                text_block = self._build_text_attachment_block(file_path, filename)
                blocks.append(text_block or f"[attachment: {file_path.name}]")
            except Exception as e:
                logger.warning("Failed to download Discord attachment: {}", e)
                blocks.append(f"[attachment: {filename} - download failed]")

        return media_paths, blocks

    @staticmethod
    def _build_text_attachment_block(path: Path, filename: str) -> str | None:
        """Render a small text-like attachment inline for the model."""
        mime = mimetypes.guess_type(filename)[0] or mimetypes.guess_type(str(path))[0]
        is_text = (mime or "").startswith("text/") or mime in TEXT_ATTACHMENT_MIME_TYPES or path.suffix.lower() in TEXT_ATTACHMENT_SUFFIXES
        if not is_text:
            return None
        ...
        truncated_by_size = len(raw) > MAX_INLINE_TEXT_ATTACHMENT_BYTES
        if truncated_by_size:
            raw = raw[:MAX_INLINE_TEXT_ATTACHMENT_BYTES]

        text = raw.decode("utf-8", errors="replace").strip()
        truncated_by_chars = len(text) > MAX_INLINE_TEXT_ATTACHMENT_CHARS
        if truncated_by_chars:
            text = text[:MAX_INLINE_TEXT_ATTACHMENT_CHARS].rstrip()
        if not text:
            return f"[attachment text: {filename} - empty]"

        suffix = ""
        if truncated_by_size or truncated_by_chars:
            suffix = "\n[attachment text truncated]"
        return f"[attachment text: {filename}]\n{text}{suffix}"

    @staticmethod
    def _compose_inbound_content(content: str, attachment_blocks: list[str]) -> str:
        """Combine message text with attachment markers and inlined text attachments."""
    ...
    def _build_inbound_metadata(message: discord.Message) -> dict[str, str | None]:
        ...
        sender_name = getattr(message.author, "display_name", None) or getattr(message.author, "name", None)
        return {
