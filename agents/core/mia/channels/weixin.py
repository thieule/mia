...
from mia.config.paths import get_media_dir, get_runtime_subdir
...
class WeixinConfig(Base):
    ...
    state_dir: str = ""  # Default: ~/.mia/weixin/
...
class WeixinChannel(BaseChannel):
    ...
    async def start(self) -> None:
        ...
        if self.config.token:
            ...
            if not await self._qr_login():
                logger.error("WeChat login failed. Run 'mia channels login weixin' to authenticate.")
                self._running = False
                return
    ...
    async def _send_text(
        ...
        client_id = f"mia-{uuid.uuid4().hex[:12]}"
