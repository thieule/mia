...
from mia import __logo__, __version__
...
from mia.cli.stream import StreamRenderer, ThinkingSpinner
from mia.config.paths import get_workspace_path, is_default_workspace
from mia.config.schema import Config
from mia.utils.helpers import sync_workspace_templates
from mia.utils.restart import (
...
app = typer.Typer(
    ...
    help=f"{__logo__} mia - Personal AI Assistant",
...
def _init_prompt_session() -> None:
    ...
    from mia.config.paths import get_cli_history_path
...
def _print_agent_response(
    ...
    console.print()
    console.print(f"[cyan]{__logo__} mia[/cyan]")
    console.print(body)
    console.print()
...
async def _print_interactive_response(
    ...
    def _write() -> None:
        ...
        ansi = _render_interactive_ansi(
            lambda c: (
                c.print(),
                c.print(f"[cyan]{__logo__} mia[/cyan]"),
...
def version_callback(value: bool):
    if value:
        console.print(f"{__logo__} mia v{__version__}")
        raise typer.Exit()
...
def onboard(
    ...
    """Initialize mia configuration and workspace."""
    from mia.config.loader import get_config_path, load_config, save_config, set_config_path
    from mia.config.schema import Config
    ...
    if wizard:
        from mia.cli.onboard import run_onboard
        ...
        try:
            ...
            console.print("[yellow]Please run 'mia onboard' again to complete setup.[/yellow]")
            raise typer.Exit(1)
    ...
    agent_cmd = 'mia agent -m "Hello!"'
    gateway_cmd = "mia gateway"
    if config:
        agent_cmd += f" --config {config_path}"
        gateway_cmd += f" --config {config_path}"

    console.print(f"\n{__logo__} mia is ready!")
    console.print("\nNext steps:")
    if wizard:
    ...
    console.print(
        "\n[dim]Want Telegram/WhatsApp? See: https://github.com/floware/mias#-chat-apps[/dim]"
    )
...
def _onboard_plugins(config_path: Path) -> None:
    ...
    from mia.channels.registry import discover_all
...
def _make_provider(config: Config):
    ...
    """
    from mia.providers.base import GenerationSettings
    ...
    if backend == "azure_openai":
        if not p or not p.api_key or not p.api_base:
            ...
            console.print("Set them in ~/.mia/config.json under providers.azure_openai section")
        ...
        if needs_key and not exempt:
            ...
            if pn == "openrouter":
                console.print("Example: set `OPENROUTER_API_KEY` in `agents/ai-tech/.env` (see `EXAMPLE_.env`).")
            elif pn == "gemini":
    ...
    if backend == "openai_codex":
        ...
        from mia.providers.azure_openai_provider import AzureOpenAIProvider
        ...
        from mia.providers.anthropic_provider import AnthropicProvider
...
def _migrate_cron_store(config: "Config") -> None:
    ...
    from mia.config.paths import get_cron_dir
...
def serve(
    ...
    verbose: bool = typer.Option(False, "--verbose", "-v", help="Show mia runtime logs"),
    workspace: str | None = typer.Option(None, "--workspace", "-w", help="Workspace directory"),
    config: str | None = typer.Option(None, "--config", "-c", help="Path to config file"),
):
    ...
    try:
        ...
        console.print("[red]aiohttp is required. Install with: pip install 'mia-ai[api]'[/red]")
        raise typer.Exit(1)

    from loguru import logger
    from mia.agent.loop import AgentLoop
    from mia.api.server import create_app
    ...
    agent_loop = AgentLoop(
        ...
        text_only_tool_nudge_rounds=runtime_config.agents.defaults.text_only_tool_nudge_rounds,
    )
...
def gateway(
    ...
    from mia.heartbeat.service import HeartbeatService
    from mia.session.manager import SessionManager
    ...
    agent = AgentLoop(
        ...
        text_only_tool_nudge_rounds=config.agents.defaults.text_only_tool_nudge_rounds,
    )
...
def agent(
    ...
    logs: bool = typer.Option(False, "--logs/--no-logs", help="Show mia runtime logs during chat"),
):
    ...
    if message:
        ...
        from mia.bus.events import InboundMessage
...
def channels_status(
    ...
    """Show channel status."""
    from mia.channels.registry import discover_all
    from mia.config.loader import load_config, set_config_path
...
def _get_bridge_dir() -> Path:
    ...
    from mia.config.paths import get_bridge_install_dir
    ...
    pkg_bridge = Path(__file__).parent.parent / "bridge"  # mia/bridge (installed)
    ...
    if not source:
        ...
        console.print("Try reinstalling: pip install --force-reinstall mia-ai")
        raise typer.Exit(1)
...
def plugins_list():
    ...
    from mia.channels.registry import discover_all, discover_channel_names
...
def status():
    """Show mia status."""
    from mia.config.loader import get_config_path, load_config

    config_path = get_config_path()
    config = load_config()
    workspace = config.workspace_path

    console.print(f"{__logo__} mia Status\n")

    console.print(f"Config: {config_path} {'[green]✓[/green]' if config_path.exists() else '[red]✗[/red]'}")
    console.print(f"Workspace: {workspace} {'[green]✓[/green]' if workspace.exists() else '[red]✗[/red]'}")

    if config_path.exists():
        from mia.providers.registry import PROVIDERS
...
def _login_github_copilot() -> None:
    try:
        from mia.providers.github_copilot_provider import login_github_copilot
