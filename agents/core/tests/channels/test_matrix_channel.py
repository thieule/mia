...
from mia.channels.matrix import _build_matrix_text_content

import mia.channels.matrix as matrix_module
from mia.bus.events import OutboundMessage
from mia.bus.queue import MessageBus
from mia.channels.matrix import (
    MATRIX_HTML_FORMAT,
    TYPING_NOTICE_TIMEOUT_MS,
    MatrixChannel,
)
from mia.channels.matrix import MatrixConfig
...
async def test_start_skips_load_store_when_device_id_missing(
    ...
    monkeypatch.setattr("mia.channels.matrix.get_data_dir", lambda: tmp_path)
    monkeypatch.setattr(
        "mia.channels.matrix.AsyncClientConfig",
        lambda **kwargs: SimpleNamespace(**kwargs),
    )
    monkeypatch.setattr("mia.channels.matrix.AsyncClient", _fake_client)
    monkeypatch.setattr(
        "mia.channels.matrix.asyncio.create_task", _fake_create_task
    )
