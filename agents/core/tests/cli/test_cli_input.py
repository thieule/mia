...
from mia.cli import stream as stream_mod
...
def mock_prompt_session():
    ...
    with patch("mia.cli.commands._PROMPT_SESSION", mock_session), \
         patch("mia.cli.commands.patch_stdout"):
        yield mock_session
...
def test_init_prompt_session_creates_session():
    ...
    with patch("mia.cli.commands.PromptSession") as MockSession, \
         patch("mia.cli.commands.FileHistory") as MockHistory, \
...
async def test_print_interactive_progress_line_pauses_spinner_before_printing():
    ...
    with patch("mia.cli.commands._print_interactive_line", side_effect=fake_print):
        thinking = stream_mod.ThinkingSpinner(console=mock_console)
        with thinking:
