"""Tests for the Mia programmatic facade."""
...
def test_from_config_missing_file():
    with pytest.raises(FileNotFoundError):
        Mia.from_config("/nonexistent/config.json")


def test_from_config_creates_instance(tmp_path):
    config_path = _write_config(tmp_path)
    bot = Mia.from_config(config_path, workspace=tmp_path)
...
def test_from_config_default_path():
    ...
    with patch("mia.config.loader.load_config") as mock_load, \
         patch("mia.facade._make_provider") as mock_prov:
...
def test_import_from_top_level():
    from mia import Mia as N, RunResult as R
    assert N is Mia
