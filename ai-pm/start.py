#!/usr/bin/env python3
"""Start the Mia PM deployment (`ai-pm/`): loads .env, installs mia-ai + ai-tools, runs gateway.
...
Layout (siblings under the repo root):
  ...
  ai-pm/      # this folder — Mia PM config + workspace + launcher
...
Canonical policies for this Mia PM deployment. The agent must not modify these files without explicit approval.
"""
...
def _require_python() -> None:
    if sys.version_info < (3, 11):
        print("Mia PM requires Python 3.11+. Current:", sys.version, file=sys.stderr)
        sys.exit(1)
...
def _ensure_test_runs_path() -> None:
    ...
    # .env may set TEST_RUNS_PATH_MIA_PM= (empty); setdefault would not override that.
    if not os.environ.get("TEST_RUNS_PATH_MIA_PM", "").strip():
        os.environ["TEST_RUNS_PATH_MIA_PM"] = default
...
def run_pip_install(quiet: bool) -> None:
    ...
    if AI_TOOLS_ROOT.is_dir():
        print("Installing ai-tools (Mia PM default; registry + pytest MCP)...", flush=True)
...
def validate_config() -> None:
    ...
    print("Config OK (Mia PM)", flush=True)
    ...
    if not dc:
        ...
        if not dc.get("enabled"):
            print("  discord token (Mia PM): skipped (disabled)", flush=True)
        else:
            print("  discord token (Mia PM):", f"set ({len(tok)} chars)" if tok else "MISSING", flush=True)
...
def main() -> None:
    _require_python()
    parser = argparse.ArgumentParser(description="Start Mia PM gateway (mia + optional Discord).")
    parser.add_argument(
        "--validate-only",
        action="store_true",
    ...
    print("Starting Mia PM gateway (Ctrl+C to stop)...", flush=True)
    run_gateway()
