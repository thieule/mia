...
from mia.channels.discord import (
    MAX_MESSAGE_LEN,
    DiscordBotClient,
    DiscordChannel,
    DiscordConfig,
...
async def test_start_returns_when_discord_dependency_missing(monkeypatch) -> None:
    ...
    monkeypatch.setattr("mia.channels.discord.DISCORD_AVAILABLE", False)
...
async def test_start_handles_client_construction_failure(monkeypatch) -> None:
    ...
    monkeypatch.setattr("mia.channels.discord.DiscordBotClient", _boom)
...
async def test_start_handles_client_start_failure(monkeypatch) -> None:
    ...
    monkeypatch.setattr("mia.channels.discord.DiscordBotClient", _FakeDiscordClient)
...
async def test_on_message_downloads_attachments(tmp_path, monkeypatch) -> None:
    ...
    channel._handle_message = capture_handle  # type: ignore[method-assign]
    monkeypatch.setattr("mia.channels.discord.get_media_dir", lambda _name: tmp_path)
...
@pytest.mark.asyncio
async def test_on_message_inlines_text_attachment_content(tmp_path, monkeypatch) -> None:
    # Small text attachments should be inlined into the forwarded prompt content.
    channel = DiscordChannel(DiscordConfig(enabled=True, allow_from=["*"]), MessageBus())
    handled: list[dict] = []
    ...
    await channel._on_message(
        _make_message(
            attachments=[_FakeAttachment(12, "notes.txt")],
            content="please review",
        )
    )

    assert len(handled) == 1
    assert handled[0]["media"] == [str(tmp_path / "12_notes.txt")]
    assert "[attachment text: notes.txt]" in handled[0]["content"]
    assert "attachment" in handled[0]["content"]
...
async def test_send_delta_streams_by_editing_message(monkeypatch) -> None:
    ...
    monkeypatch.setattr("mia.channels.discord.time.monotonic", lambda: next(times, 5.0))
...
async def test_send_delta_stream_end_splits_oversized_reply(monkeypatch) -> None:
    ...
    times = iter([1.0, 3.0])
    monkeypatch.setattr("mia.channels.discord.time.monotonic", lambda: next(times, 3.0))
