...
try:
    from mia.channels import dingtalk
...
import mia.channels.dingtalk as dingtalk_module
from mia.channels.dingtalk import DingTalkChannel, DingTalkStreamHandler
from mia.channels.dingtalk import DingTalkConfig
...
async def test_handler_uses_voice_recognition_text_when_text_is_empty(monkeypatch) -> None:
    ...
    handler = DingTalkStreamHandler(channel)
...
async def test_handler_processes_file_message(monkeypatch) -> None:
    ...
    async def fake_download(download_code, filename, sender_id):
        return f"/tmp/mia_dingtalk/{sender_id}/{filename}"
    ...
    assert "/tmp/mia_dingtalk/user1/report.xlsx" in msg.content
...
async def test_download_dingtalk_file(tmp_path, monkeypatch) -> None:
    ...
    monkeypatch.setattr(
        "mia.config.paths.get_media_dir",
