...
def test_openrouter_sets_default_attribution_headers() -> None:
    spec = find_by_name("openrouter")
    with patch("mia.providers.openai_compat_provider.AsyncOpenAI") as MockClient:
    ...
    assert headers["HTTP-Referer"] == "https://github.com/floware/mias"
    assert headers["X-OpenRouter-Title"] == "mia"
...
def test_openrouter_user_headers_override_default_attribution() -> None:
    ...
    with patch("mia.providers.openai_compat_provider.AsyncOpenAI") as MockClient:
        OpenAICompatProvider(
            ...
            default_model="anthropic/claude-sonnet-4-5",
            extra_headers={
                "HTTP-Referer": "https://example.com/custom",
                "X-OpenRouter-Title": "mia Pro",
    ...
    headers = MockClient.call_args.kwargs["default_headers"]
    assert headers["HTTP-Referer"] == "https://example.com/custom"
    assert headers["X-OpenRouter-Title"] == "mia Pro"
    assert headers["X-OpenRouter-Categories"] == "cli-agent,personal-agent"
