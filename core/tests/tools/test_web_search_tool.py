...
from mia.agent.tools.web import WebSearchTool
...
async def test_brave_search(monkeypatch):
    ...
    result = await tool.execute(query="mia", count=1)
...
async def test_duckduckgo_search(monkeypatch):
    ...
    monkeypatch.setattr("mia.agent.tools.web.DDGS", MockDDGS, raising=False)
    import mia.agent.tools.web as web_mod
