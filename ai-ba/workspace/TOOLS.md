...
- Prefer **canonical specs in Git** when MCP exposes repo paths (see playbook); otherwise drafts under **`agent/`** (and `admin/` only when policy allows).
...
- **`spawn`** — parallel research branches when **`admin/PRE_IMPLEMENTATION_APPROVAL.md`** allows; consolidate into one narrative.
