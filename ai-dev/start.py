#!/usr/bin/env python3
"""Start the Mia dev deployment (`ai-dev/`): loads .env, installs mia-ai + ai-tools, runs gateway.
...
Layout (siblings under the repo root):
  ...
  ai-dev/      # this folder — Mia dev config + workspace + launcher
...
Canonical policies for this Mia dev deployment. The agent must not modify these files without explicit approval.
"""
...
def _require_python() -> None:
    if sys.version_info < (3, 11):
        print("Mia dev requires Python 3.11+. Current:", sys.version, file=sys.stderr)
        sys.exit(1)
...
def _ensure_test_runs_path() -> None:
    ...
    # .env may set TEST_RUNS_PATH_MIA_DEV= (empty); setdefault would not override that.
    if not os.environ.get("TEST_RUNS_PATH_MIA_DEV", "").strip():
        os.environ["TEST_RUNS_PATH_MIA_DEV"] = default
...
def run_pip_install(quiet: bool) -> None:
    ...
    if AI_TOOLS_ROOT.is_dir():
        print("Installing ai-tools (Mia dev default; registry + pytest MCP)...", flush=True)
...
def validate_config() -> None:
    ...
    print("Config OK (Mia dev)", flush=True)
    ...
    if not dc:
        ...
        if not dc.get("enabled"):
            print("  discord token (Mia dev): skipped (disabled)", flush=True)
        else:
            print("  discord token (Mia dev):", f"set ({len(tok)} chars)" if tok else "MISSING", flush=True)
...
def main() -> None:
    _require_python()
    parser = argparse.ArgumentParser(description="Start Mia dev gateway (mia + optional Discord).")
    parser.add_argument(
        "--validate-only",
        action="store_true",
    ...
    print("Starting Mia dev gateway (Ctrl+C to stop)...", flush=True)
    run_gateway()
