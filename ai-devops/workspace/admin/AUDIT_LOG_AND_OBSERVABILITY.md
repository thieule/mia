# Audit trail and observability (Mia DevOps)

**Status:** Human-maintained policy under `admin/`. Describes **what Mia DevOps / nanobot already records** and **how admins can review** it. Mia DevOps **must not** edit files in `admin/` without an approved workflow.
...
- **Retention:** `sessions/` is often **gitignored** (see `ai-devops/.gitignore`); copy snapshots to secure storage if you need **long-term compliance** archives.
