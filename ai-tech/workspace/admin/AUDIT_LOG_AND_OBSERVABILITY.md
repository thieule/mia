# Audit trail and observability (Mia tech)

**Status:** Human-maintained policy under `admin/`. Describes **what Mia tech / nanobot already records** and **how admins can review** it. Mia tech **must not** edit files in `admin/` without an approved workflow.
...
- **Retention:** `sessions/` is often **gitignored** (see `ai-tech/.gitignore`); copy snapshots to secure storage if you need **long-term compliance** archives.
